@ Filename :    SBSchwalb.s
@ Author   :    Joseph Schwalb
@ Email    :    jds0099@uah.edu
@ CS309-01 :    2020
@ Purpose  :    ARM Input (ARM3) Assignment. Simple input checking
@               program.
@ 
@ History  : 
...
@ Set up r0 with the address of input pattern.
@ scanf puts the input value at the address stored in r1. We are going
@ to use the address for our declared variable in the data section - charInput. 
@ After the call to scanf the input is at the address pointed to by r1 which 
@ in this case will be charInput. 
@ it can be printed. 
   ...
   beq char_readerror             @ If there was a read error go handle it. 
...
@***********
   ...
   ldr r0, =isSpecialChar
   bl  printf

   b   myexit @ leave the code. 

@***********
determine_type_letter:
@***********
   cmp r1, #0x5B
   blt uppercase_letter
   cmp r1, #0x60
...
.balign 4
strLessThan: .asciz "The number is less than 100 \n"

.balign 4
strGreaterThanOrEqual: .asciz "The number is greater than or equal to 100 \n"

.balign 4
isSpecialChar: .asciz "The input is a special character \n"

.balign 4
isUpperCaseLetter: .asciz "The input is an uppercase letter \n"

.balign 4
isLowerCaseLetter: .asciz "The input is a lowercase letter \n"
